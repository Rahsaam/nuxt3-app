<template>
    <section class="single_page_section layout_padding">
        <div class="container">
            <div class="row">
                <div class="col-md-10 offset-md-1">
                    <div class="row gy-5">
                        <div class="col-sm-12 col-lg-6">
                            <h3 class="fw-bold mb-4">{{ product.name }}</h3>
                            <h5 v-if="product.is_sale" class="mb-3">
                                <del>{{ priceFormat(product.price) }}</del>
                                {{ priceFormat(product.sale_price) }}
                                تومان
                                <div class="text-danger fs-6">
                                    {{ salePercent(product.price, product.sale_price) }}% تخفیف
                                </div>
                            </h5>
                            <h5 v-else>
                                {{ priceFormat(product.price) }}
                                تومان
                            </h5>
                            <p>{{ product.description }}</p>

                            <div class="mt-5 d-flex">
                                <button @click="addToCart(product)" class="btn-add">افزودن به سبد خرید</button>
                                <div class="input-counter ms-4">
                                    <span class="plus-btn" @click="increase">
                                        +
                                    </span>
                                    <div class="input-number">{{ quantity }}</div>
                                    <span class="minus-btn" @click="decrease">
                                        -
                                    </span>
                                </div>
                            </div>
                        </div>
                        <div class="col-sm-12 col-lg-6">
                            <div id="carouselExampleIndicators" class="carousel slide" data-bs-ride="carousel">
                                <div class="carousel-indicators">
                                    <button type="button" data-bs-target="#carouselExampleIndicators"
                                        data-bs-slide-to="0" class="active" aria-current="true"
                                        aria-label="Slide 1"></button>
                                    <button v-for="(image, index) in product.images" :key="image.id" type="button" data-bs-target="#carouselExampleIndicators"
                                        :data-bs-slide-to="`${index + 1}`"></button>
                                </div>
                                <div class="carousel-inner">
                                    <div class="carousel-item active">
                                        <img class="d-block w-100" src="/images/preloader.png" width="464" height="309" 
                                        v-img="product.primary_image" />
                                    </div>
                                    <div v-for="image in product.images" :key="image.id" class="carousel-item">
                                        <img class="d-block w-100" src="/images/preloader.png" width="464" height="309" 
                                        v-img="image.image" />
                                    </div>
                                </div>
                                <button class="carousel-control-prev" type="button"
                                    data-bs-target="#carouselExampleIndicators" data-bs-slide="prev">
                                    <span class="carousel-control-prev-icon" aria-hidden="true"></span>
                                    <span class="visually-hidden">Previous</span>
                                </button>
                                <button class="carousel-control-next" type="button"
                                    data-bs-target="#carouselExampleIndicators" data-bs-slide="next">
                                    <span class="carousel-control-next-icon" aria-hidden="true"></span>
                                    <span class="visually-hidden">Next</span>
                                </button>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </section>
</template>

<script setup>
const props = defineProps(['product']);
import {useStateCart} from '../../store/counter';
const cart = useStateCart();
const quantity = ref(1);

function addToCart (product) {
    cart.remove(product.id);
    cart.addToCart(product, quantity.value);
}

function increase () {
    if(quantity.value < props.product.quantity) quantity.value += 1;
}
function decrease () {
    return quantity.value === 1 ? 1 : quantity.value -= 1;
}
</script>

<style lang="scss" scoped>

</style>\